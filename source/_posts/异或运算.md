...
异或是一个数学运算符，英文为 exclusive OR，缩写成 XOR。异或的数学符号为「$\bigoplus$」，计算机符号为「XOR」。如果二进制数 a 和 b 同位置不相同，则该位置的结果为 1，反之为 0。
...
2. 恒等律：任何数与 0 异或都等于其本身，即 $a \bigoplus 0=0$
...
假设一个集合中有 $n$ 个数，范围为 $[1, n]$，其中有 $n - 2$ 个互不相同的数，有一个数重复一次，设计一个 $O(n)$ 时间复杂度，$O(1)$ 空间复杂度的算法，找出这个数。

推导：
1. 假设不包含重复的第二个数的所有数的异或的结果为 $1 \bigoplus2 \bigoplus ... \bigoplus n = T$；
2. 那么，包含第二个重复的数的所有数的异或结果为 $1 \bigoplus2 \bigoplus ... \bigoplus n \bigoplus x = T \bigoplus x$。
